...
class DiffusionVLQwenVLForCausalLM_Base(Qwen2_5_VLPreTrainedModel):
    ...
    def _apply_bd3lm_noise_embedding(self, inputs_embeds, labels):
        """Apply causal block-wise BD3-LM noise in embedding space.

        The input/output format is unchanged:
            return bd3lm_inputs, move_indices, loss_scale, inputs_embeds, avg_noise_level

        Difference from the original:
            - Original: each valid token is independently masked by Bernoulli(p).
            - New: within each block, masked tokens must form a right-side suffix.
            That is, no masked supervised token can appear to the left of an
            unmasked supervised token inside the same block.
        """
        batch_size, seq_len, hidden_size = inputs_embeds.shape
        device = inputs_embeds.device
        block_size = self.model.bd3lm_block_size
        ...
        # ------------------------------------------------------------------
        # 1. Sample diffusion time per block, same as original code
        # ------------------------------------------------------------------
        _eps_b = torch.rand((batch_size, num_blocks), device=device)
        ...
        # ------------------------------------------------------------------
        # 2. Build causal suffix mask inside each block
        # ------------------------------------------------------------------
        text_token_mask = labels != IGNORE_INDEX

        # Position information
        token_pos = torch.arange(seq_len, device=device)                      # [seq_len]
        block_ids = token_pos // block_size                                   # [seq_len]
        pos_in_block = token_pos % block_size                                 # [seq_len]

        # Real length of each block. The last block may be shorter than block_size.
        block_starts = torch.arange(num_blocks, device=device) * block_size    # [num_blocks]
        block_lens = (seq_len - block_starts).clamp(min=0, max=block_size)     # [num_blocks]

        # p is constant inside each block because t was repeat_interleave'd.
        # Take the p value at each block start.
        p_block = p[:, block_starts]                                           # [batch_size, num_blocks]

        # Convert noise probability p into number of masked tokens in each block.
        #
        # num_mask_per_block ranges from 0 to block_len.
        # Using block_len + 1 makes full-block mask possible when p is high.
        #
        # Examples for block_len=8:
        #   num_mask=0  -> no token masked in this block
        #   num_mask=3  -> last 3 positions masked
        #   num_mask=8  -> entire block masked
        num_mask_per_block = torch.floor(
            p_block * (block_lens.to(dtype=p_block.dtype).unsqueeze(0) + 1.0)
        ).to(torch.long)

        num_mask_per_block = torch.minimum(
            num_mask_per_block,
            block_lens.unsqueeze(0)
        )

        # cut is the first masked position inside each block.
        #
        # block_len=8, num_mask=3 -> cut=5
        # positions: 0 1 2 3 4 | 5 6 7
        #           keep        | mask
        cut_per_block = block_lens.unsqueeze(0) - num_mask_per_block           # [batch_size, num_blocks]

        # Expand block-level cut to token-level cut.
        cut_per_token = cut_per_block[:, block_ids]                            # [batch_size, seq_len]

        # suffix_mask[b, i] = True iff token i is on the right side of the cut
        # inside its block.
        suffix_mask = pos_in_block.unsqueeze(0) >= cut_per_token               # [batch_size, seq_len]

        # Only supervised text tokens are allowed to be masked.
        # Prompt/image/padding tokens remain unchanged.
        move_indices = suffix_mask & text_token_mask

        # ------------------------------------------------------------------
        # 3. Optional safety fallback:
        #    If a sample has valid labels but no masked token, mask its rightmost
        #    valid token. This preserves the causal suffix property.
        # ------------------------------------------------------------------
        has_valid = text_token_mask.any(dim=1)                                 # [batch_size]
        has_move = move_indices.any(dim=1)                                     # [batch_size]
        need_force = has_valid & (~has_move)                                   # [batch_size]

        if need_force.any():
            token_pos_2d = token_pos.unsqueeze(0).expand(batch_size, -1)        # [batch_size, seq_len]

            # Rightmost valid supervised token per sample.
            last_valid_pos = torch.where(
                text_token_mask,
                token_pos_2d,
                torch.full_like(token_pos_2d, -1)
            ).max(dim=1).values                                                # [batch_size]

            force_mask = token_pos_2d == last_valid_pos.unsqueeze(1)
            move_indices = move_indices | (need_force.unsqueeze(1) & force_mask)

        # ------------------------------------------------------------------
        # 4. Replace selected tokens with mask embedding
        # ------------------------------------------------------------------
        mask_embed = self.get_input_embeddings()(torch.tensor([151671], device=device))
        xt_embeds = torch.where(move_indices.unsqueeze(-1), mask_embed, inputs_embeds)

        # avg_noise_level now reflects actual mask ratio, not raw p mean.
        avg_noise_level = move_indices.float().mean().item()
    ...
    #     return loss
    # def _compute_bd3lm_loss_embedding(self, logits, labels, move_indices, loss_scale=None):
    #     """Compute CSL-weighted CE loss on masked positions only.

    #     Compatible with causal suffix move_indices:
    #         - each block may have a different number of masked tokens
    #         - masked tokens are on the right side of each block
    #         - blocks may have no masked tokens or be fully masked

    #     CSL rule:
    #         In each block, masked tokens are ordered as m1, m2, m3, ...

    #         weight(m1) = 1
    #         weight(m2) = reliability(m1)
    #         weight(m3) = reliability(m1) * reliability(m2)
    #         ...

    #         reliability(mi) =
    #             1.0       if P(label_i) >= threshold
    #             P(label_i) otherwise
    #     """
    #     masked_positions = move_indices & (labels != IGNORE_INDEX)
    ...
    #     csl_threshold = getattr(self.config, "csl_threshold", 0.9)
    #     csl_min_weight = getattr(self.config, "csl_min_weight", 0.0)

    #     # ------------------------------------------------------------
    #     # 1. Compute CE and gold-label confidence for every position
    #     # ------------------------------------------------------------
    ...
    #     token_loss_all = -gold_log_probs
    #     gold_probs_all = gold_log_probs.exp()

    #     # ------------------------------------------------------------
    #     # 2. Convert gold confidence into CSL reliability
    #     # ------------------------------------------------------------
    #     reliability = torch.where(
    #         gold_probs_all >= csl_threshold,
    #         torch.ones_like(gold_probs_all),
    #         gold_probs_all,
    #     )

    #     # Do not let later-token losses optimize previous-token confidence
    #     # through the CSL weight path.
    #     reliability = reliability.detach()

    #     # Non-masked positions should not affect cumulative reliability.
    #     reliability = torch.where(
    #         masked_positions,
    #         reliability,
    ...
    #     # ------------------------------------------------------------
    #     # 3. Compute cumulative reliability inside each block
    #     # ------------------------------------------------------------
    #     num_blocks = (seq_len + block_size - 1) // block_size
    ...
    #     if pad_len > 0:
    #         reliability_padded = F.pad(reliability, (0, pad_len), value=1.0)
    #         masked_padded = F.pad(masked_positions, (0, pad_len), value=False)
    #     else:
    #         reliability_padded = reliability
    #         masked_padded = masked_positions

    #     reliability_blocks = reliability_padded.view(batch_size, num_blocks, block_size)

    #     # Product of reliabilities from left to right inside each block.
    #     cum_reliability = torch.cumprod(reliability_blocks, dim=-1)

    #     # Shift right so current token's loss is weighted only by previous
    #     # masked tokens, not by its own confidence.
    #     prev_cum_reliability = torch.cat(
    #         [
    #             torch.ones_like(cum_reliability[..., :1]),
    #             cum_reliability[..., :-1],
    #         ],
    #         dim=-1,
    #     )

    #     csl_weights = prev_cum_reliability.view(batch_size, padded_len)[:, :seq_len]

    #     if csl_min_weight > 0:
    #         csl_weights = csl_weights.clamp_min(csl_min_weight)

    #     # ------------------------------------------------------------
    #     # 4. Apply CSL weights to masked-token CE loss
    #     # ------------------------------------------------------------
    #     token_loss = token_loss_all[masked_positions]
    #     token_weight = csl_weights[masked_positions]

    #     weighted_token_loss = token_loss * token_weight

    #     # Same per-sample normalization style as your previous version.
    #     valid_counts = masked_positions.sum(dim=1).clamp_min(1)
    #     per_token_denominator = valid_counts.unsqueeze(1).expand_as(labels)[masked_positions]

    #     loss = torch.sum(weighted_token_loss / per_token_denominator) / labels.shape[0]

    #     return loss
    
    # 上下两个阈值
    def _compute_bd3lm_loss_embedding(self, logits, labels, move_indices, loss_scale=None):
        """Compute CSL-weighted CE loss with hard prefix gate.

        Rules:
            1. Loss is computed only on move_indices & valid labels.
            2. CSL soft weight:
                weight(m1) = 1
                weight(m2) = reliability(m1)
                weight(m3) = reliability(m1) * reliability(m2)
                ...
            3. Hard prefix gate:
                if any previous masked token in the same block has
                P(label) < csl_stop_threshold, the current token is excluded.
            4. Excluded tokens do not participate in numerator or denominator.
        """
        masked_positions = move_indices & (labels != IGNORE_INDEX)
        ...
        # High threshold:
        # if confidence >= csl_threshold, reliability = 1.0
        csl_threshold = getattr(self.config, "csl_threshold", 0.95)

        # Low threshold:
        # if confidence < csl_stop_threshold, later tokens in the block are excluded.
        csl_stop_threshold = getattr(self.config, "csl_stop_threshold", 0.9)
        ...
        # 3. Hard prefix gate
        # ------------------------------------------------------------
        # A masked token is "good enough to allow future tokens" if its
        # gold confidence >= csl_stop_threshold.
        #
        # Non-masked positions are treated as good because they should not stop
        # the masked suffix training.
        hard_good = torch.where(
            masked_positions,
            gold_probs_all.detach() >= csl_stop_threshold,
            torch.ones_like(masked_positions, dtype=torch.bool),
        )

        # ------------------------------------------------------------
        # 4. Pad to block shape
        # ------------------------------------------------------------
        num_blocks = (seq_len + block_size - 1) // block_size
        padded_len = num_blocks * block_size
        pad_len = padded_len - seq_len
        ...
        if pad_len > 0:
            reliability_padded = F.pad(reliability, (0, pad_len), value=1.0)
            hard_good_padded = F.pad(hard_good, (0, pad_len), value=True)
            masked_padded = F.pad(masked_positions, (0, pad_len), value=False)
        else:
            reliability_padded = reliability
            hard_good_padded = hard_good
            masked_padded = masked_positions

        reliability_blocks = reliability_padded.view(batch_size, num_blocks, block_size)
        hard_good_blocks = hard_good_padded.view(batch_size, num_blocks, block_size)
        masked_blocks = masked_padded.view(batch_size, num_blocks, block_size)

        # ------------------------------------------------------------
        # 5. Soft CSL cumulative weight inside each block
        # ------------------------------------------------------------
        cum_reliability = torch.cumprod(reliability_blocks, dim=-1)

        # Shift right:
        # current token's loss is weighted only by previous masked tokens,
        # not by its own confidence.
        prev_cum_reliability = torch.cat(
            [
                torch.ones_like(cum_reliability[..., :1]),
                cum_reliability[..., :-1],
        ...
        # ------------------------------------------------------------
        # 6. Hard gate active positions
        # ------------------------------------------------------------
        # hard_prefix_ok[i] = all previous masked tokens are good.
        #
        # Example:
        #   hard_good = [T, F, T, T]
        #   cum       = [T, F, F, F]
        #   shifted   = [T, T, F, F]
        #
        # Thus the low-confidence token itself is still trained,
        # but tokens to its right are excluded.
        hard_cum = torch.cumprod(hard_good_blocks.to(torch.int64), dim=-1).bool()

        hard_prefix_ok = torch.cat(
            [
                torch.ones_like(hard_cum[..., :1]),
                hard_cum[..., :-1],
            ],
            dim=-1,
        )

        hard_prefix_ok = hard_prefix_ok.view(batch_size, padded_len)[:, :seq_len]

        active_positions = masked_positions & hard_prefix_ok

        if not active_positions.any():
            return torch.tensor(0.0, device=logits.device, requires_grad=True)

        # ------------------------------------------------------------
        # 7. Apply CSL weights only to active positions
        # ------------------------------------------------------------
        token_loss = token_loss_all[active_positions]
        token_weight = csl_weights[active_positions]

        weighted_token_loss = token_loss * token_weight

        # Denominator excludes hard-gated positions, as you requested.
        active_counts = active_positions.sum(dim=1).clamp_min(1)
        per_token_denominator = active_counts.unsqueeze(1).expand_as(labels)[active_positions]
